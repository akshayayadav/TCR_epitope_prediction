...
vdjcdr3_emb_dict = read_h5py_embeddings("jh_data/T5_embeddings/score_thresh1/V-CDR3-J_embedding.h5")
epi_emb_dict = read_h5py_embeddings("jh_data/T5_embeddings/score_thresh1/Epitope_embedding.h5")
...
for model_part in range(1, 21):
    ...
    with torch.no_grad():
        ...
        for batch_t in range(0, test_vdj_epi_df.shape[0], batch_size):
            ...
            b_X_test_vdj, b_X_test_epi, _ = assign_embeddings_per_residue(vdjcdr3_emb_dict, epi_emb_dict, b_vdj_epi_df_test)
            b_X_test_vdj, b_X_test_epi = b_X_test_vdj.to(device), b_X_test_epi.to(device)
...
y_pred_majority = predict_class_label_by_majority(pred_dict, majority_thresh=19)
...
test_vdj_epi_df.to_csv("jh_data/jh_trb_mhc1_scorethres1_hs_pred_per_residue.csv", index=False)